> **전체 출처 기록 (보관용).** Yana Robot의 출처와 XiaoZhi AI Chatbot(MIT)과의
> 관계를 상세히 설명하는 전체 버전입니다. 저장소 루트의
> [`README.md`](../README_ko.md)에도 동일한 내용의 짧은 버전이 있습니다 —
> 여기 내용이 숨겨진 것이 아니라, 저장소 첫 화면이 Yana Wheelbot이 실제로
> 하는 일을 먼저 보여줄 수 있도록 옮겨둔 것뿐입니다.
> 참고: [English](origin-story.md) · [Tiếng Việt](origin-story_vi.md).

---

# Yana Robot

([English](origin-story.md) | [Tiếng Việt](origin-story_vi.md) | 한국어)
...
<img src="mcp-based-graph.jpg" alt="MCP로 모든 것을 제어" width="320">
...
보드 디렉토리에서 여전히 동작합니다 ([supported-boards.md](supported-boards.md) 참고).
...
- ESP-IDF v6.0 이상; v6.0.2가 권장 안정 SDK입니다. v5.5.2는 문서화된
  ...
  [ESP-IDF 6.0 마이그레이션 가이드](esp-idf-6-migration.md)를 참고하세요.
...
- [Custom Board Guide](custom-board.md) — 새 보드 만들기
- [MCP Protocol IoT Control Usage](mcp-usage.md)
- [MCP Protocol Interaction Flow](mcp-protocol.md)
- [WebSocket 프로토콜](websocket.md) · [MQTT + UDP 프로토콜](mqtt-udp.md)
- [지원 하드웨어](supported-boards.md) — 물려받은 전체 보드 목록, 기능 목록, 호환 가능한 서드파티 생태계
- [ESP-IDF 6.0 마이그레이션 가이드](esp-idf-6-migration.md)
