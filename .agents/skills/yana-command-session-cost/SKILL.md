---
name: yana-command-session-cost
description: "Yana AI /session-cost command adapter. Báo cáo token thật và ước tính chi phí từ JSONL session logs của Claude Code. Không gọi API — đọc file local. Usage — /session-cost [last|all|session <id>]"
---

# Yana AI Command: /session-cost

Invoke this workflow explicitly as `$yana-command-session-cost`.
