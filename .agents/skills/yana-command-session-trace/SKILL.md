---
name: yana-command-session-trace
description: "Yana AI /session-trace command adapter. Render an ASCII timeline of the current session — shows every tool call, hook decision, risk score, and block event in chronological order. Usage: /session-trace [--tail N | --filter block|warn|allow]"
---

# Yana AI Command: /session-trace

Invoke this workflow explicitly as `$yana-command-session-trace`.
