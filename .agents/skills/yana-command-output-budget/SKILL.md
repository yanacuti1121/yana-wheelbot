---
name: yana-command-output-budget
description: "Yana AI /output-budget command adapter. Hiển thị proxy report về output volume của session — tool calls, files đọc/ghi, budget mode status. Không đo token thật từ API. Usage — /output-budget [report|warn <kb>|reset]"
---

# Yana AI Command: /output-budget

Invoke this workflow explicitly as `$yana-command-output-budget`.
