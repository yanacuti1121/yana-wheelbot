...
bash .Codex/scripts/generate-wiki.sh --commit
```

**After major code changes:**
```bash
npx gitnexus analyze --force    # refresh index
bash .Codex/scripts/generate-wiki.sh --force --commit
```
...
```json
{
  "hooks": {
    "PostToolUse": [{
      "matcher": "Bash",
      "hooks": [{"type": "command", "command": "bash .Codex/scripts/generate-wiki.sh --commit 2>/dev/null || true"}]
    }]
  }
}
```
