---
name: yana-command-cost-report
description: "Yana AI /cost-report command adapter. Real-time token cost dashboard from token-budget.json + session JSONL. Shows per-tool call counts, circuit breaker state, loop attempts, and fast-tier activations. Usage — /cost-report [--all]"
---

# Yana AI Command: /cost-report

Invoke this workflow explicitly as `$yana-command-cost-report`.
