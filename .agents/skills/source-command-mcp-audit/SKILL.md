...
# source-command-mcp-audit

Use this skill when the user asks to run the migrated source command `mcp-audit`.
...
1. Read `.Codex/mcp-tier.json` to see current tier classification.
2. Read `.mcp.json` to see what servers are actively configured.
3. Check `/tmp/Codex-tool-attn-*.json` files for tool-attention session logs.
...
Apply these rules — read from `.Codex/mcp-tier.json` thresholds — but default to:
...
`.Codex/mcp-tier.json`? (yes/no)". Only modify config files after explicit
